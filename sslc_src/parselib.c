...
void parseLibExpression(Procedure *p, int i) {
   switch(i) {
      PARSEEXP(SOUNDPLAY, 2);
      PARSEEXP(SAYGETLASTPOS, 0);
		PARSEEXP(SELECTFILELIST, 2);
		PARSEEXP(CHECKREGION, 1);
      default: parseExtraExpression(p, i); break;
   }
}

void parseLib(Procedure *p, int i) {
   switch(i) {
      PARSE(MOUSESHAPE, 3);
      PARSE(SETGLOBALMOUSEFUNC, 4);
      PARSE(HIDEMOUSE, 0);
      PARSE(SHOWMOUSE, 0);
      PARSE(REFRESHMOUSE, 1);

      PARSE(DELETEBUTTON, 1);
      PARSE(ADDBUTTONTEXT, 2);
      PARSE(ADDBUTTONGFX, 4);
      PARSE(ADDBUTTONPROC, 5);
      PARSE(ADDBUTTONFLAG, 2);
      PARSE(ADDBUTTONRIGHTPROC, 3);
      PARSE(ADDBUTTON, 5);
      ...
      PARSE(ADDREGIONPROC, 5);
      PARSE(ADDREGIONFLAG, 2);
      PARSE(ADDREGIONRIGHTPROC, 3);
      ...
      PARSE(ACTIVATEREGION, 2);
		...
		PARSE(ADDNAMEDEVENT, 2);
		PARSE(ADDNAMEDHANDLER, 2);
		PARSE(CLEARNAMED, 1);
		PARSE(SIGNALNAMED, 1);
      ...
      default:
         parseExtraStatement(p, i);
         break;
   }
}

int writeLibStatement(NodeList *n, int i, OutputStream *f) {
