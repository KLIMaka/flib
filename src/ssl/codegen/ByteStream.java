...
public class ByteStream implements Iterable<Byte>, IByteBlock {
    ...
    public void writeLabels(int addr) {
        for (Label l : m_labels) {
            l.getCallback().setAddress(addr + l.getMark().getCluster() * m_clusterSize + l.getMark().getOffset());
        }
    }
    ...
    public byte[] getBytes() {
        ...
        for (int i = 0; i < m_currentCluster; i++) {
            System.arraycopy(m_data.get(i), 0, ret, i * m_clusterSize, m_clusterSize);
        }
        System.arraycopy(m_data.get(m_currentCluster), 0, ret, m_clusterSize * m_currentCluster, m_currentClusterOffset);
